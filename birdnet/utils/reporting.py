...
from .helpers import DB_PATH, get_font, get_settings
...
def extract(in_file, out_file, start, stop):
    result = subprocess.run(["sox", "-V1", f"{in_file}", f"{out_file}", "trim", f"={start}", f"={stop}"], check=True, capture_output=True)
...
def spectrogram(in_file, title, comment, raw=0):
    ...
    os.close(fd)
    args = ["sox", "-V1", f"{in_file}", "-n", "remix", "1", "rate", "24k", "spectrogram", "-t", "", "-c", "", "-o", tmp_file]
...
def write_to_json_file(file: ParseFileName, detections: [Detection]):
    ...
    dets = {
        ...
        "detections": [{"start": det.start, "common_name": det.common_name, "confidence": det.confidence} for det in detections],
    }
...
def bird_weather(file: ParseFileName, detections: [Detection]):
    ...
    if detections:
        ...
        soundscape_url = f"https://app.birdweather.com/api/v1/stations/{conf['BIRDWEATHER_ID']}/soundscapes?timestamp={file.iso8601}"

        try:
            response = requests.post(url=soundscape_url, data=flac_data, timeout=30, headers={"Content-Type": "audio/flac"})
